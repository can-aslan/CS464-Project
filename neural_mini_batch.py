...
# -*- coding: utf-8 -*-
"""
Created on Sat May 11 13:45:32 2024
...
m_train = len(classes_train)

for i in range(epochs):
    # Shuffle the training data for each epoch
    shuffled_indices = list(range(m_train))
    random.shuffle(shuffled_indices)

    for j in range(0, m_train, batch_size):
        batch_indices = shuffled_indices[j:j+batch_size]
        data_batch = data_train[:, batch_indices]
        classes_batch = classes_train[batch_indices]

        Z1 = W1.dot(data_batch) + b1
        A1 = sigmoid(Z1)
        ...
        dZ3 = A3 - classes_batch.to_numpy().reshape(A3.shape)
        ...
        dW1 = (1 / batch_size) * dZ1.dot(data_batch.T)
    ...
    Z_train1 = W1.dot(data_train) + b1
    A_train1 = sigmoid(Z_train1)

    Z_train2 = W2.dot(A_train1) + b2
    A_train2 = sigmoid(Z_train2)

    Z_train3 = W3.dot(A_train2) + b3
    A_train3 = sigmoid(Z_train3)

    pred_train = predict(A_train3)
    train_accuracy = accuracy_score(classes_train, pred_train.squeeze())
    ...
    print("Epoch ", i+1, "Train Accuracy = ", train_accuracy, "Val Accuracy = ", val_accuracy)
